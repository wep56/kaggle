# -*- coding: utf-8 -*-
"""
Created on Sat Apr 22 16:04:33 2017
...
def familysize():
    ...
    df['Singleton'] = df['FamilySize'].map(lambda s : 1 if s == 1 else 0).astype(np.int)
    df['SmallFamily'] = df['FamilySize'].map(lambda s : 1 if 2<=s<=4 else 0).astype(np.int)
    df['LargeFamily'] = df['FamilySize'].map(lambda s : 1 if 5>=s else 0).astype(np.int)
