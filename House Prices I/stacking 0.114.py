# -*- coding: utf-8 -*-
"""
Created on Thu Jun 29 14:05:12 2017
...
y_train = label_df.ravel()

et_oof_train,et_oof_test = get_oof(et, train_df_munged, y_train, test_df_munged)
rf_oof_train,rf_oof_test = get_oof(rf,train_df_munged, y_train, test_df_munged)
rd_oof_train,rd_oof_test = get_oof(rd,train_df_munged, y_train, test_df_munged)
ls_oof_train,ls_oof_test = get_oof(ls,train_df_munged, y_train, test_df_munged)
gb_oof_train,gb_oof_test = get_oof(gb,train_df_munged, y_train, test_df_munged)
...
gb = GradientBoostingRegressor(
 ...
 max_depth= 4,                        
 subsample=0.8,
 ).fit(train_df_munged, y_train)
y_pred = gb.predict(test_df_munged)
y_pred = np.exp(y_pred)
submission = pd.DataFrame({"Id": test_df['Id'], "SalePrice": y_pred})
submission.to_csv('stacking5.csv', index=False)
